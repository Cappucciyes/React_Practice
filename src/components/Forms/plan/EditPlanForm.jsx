...
function EditPlanForm({ planInfo, isFormOpen, setIsFormOpen }) {
  ...
  let planColorCss = useContext(PlansColorCSSContext);

  let colorChoice = [];

  for (let colorValue in planColorContext) colorChoice.push(colorValue);

  console.log(planColorCss);
  ...
  function handleSubmit(submitEvent) {
    ...
    let planListwithoutTarget = planContext.filter(
    ...
    for (let i in whenData) {
      ...
      for (let j in planListwithoutTarget) {
        // checks if plan with the same name already exists
        if (planListwithoutTarget[j].name == formObject.name) {
          alert(`Already with the plan named "${formObject.name}"`);
          return;
        }

        for (let k in planListwithoutTarget[j].whenObjects) {
          if (whenData[i].day == planListwithoutTarget[j].whenObjects[k].day) {
            let [newPlanTime, oldPlanTime] = [
              whenData[i],
              planListwithoutTarget[j].whenObjects[k],
            ];
            let overlap =
            ...
            if (overlap) {
              alert(
                `On day ${whenData[i].day}, plan overlap with plan: ${planListwithoutTarget[j].name}`
              );
              return;
            }
          }
        ...
        <form onSubmit={handleSubmit} className="bg-white p-10 w-[20%]">
          ...
          <div className="flex flex-row">
            ...
            <select
              name="color"
              id="color"
              defaultValue={planColorCss[planInfo.color]}
            >
